...
- [x] NS-20260725-08 (S) **Background tenderness sky soften** — Engine-wide: on `tenderness`, ease nebula/aurora drift and warm-dim the glow so the sky goes gentle under tender vocals, re-brightening on release. Alive across all presets. Area: `BackgroundLayer.tsx`. Accept: a tender passage visibly softens and warms the sky vs an aggressive section; gather inhale (#24), holdBreath hush (#82), leanIn pull (#65), and afterglow warmth (#45) stay distinct. — PR #101
- [x] NS-20260725-09 (S) **Ink Bloom holdBreath still water** — During `holdBreath`, suspend the plumes mid-curl and glass the surface so the ink listens, releasing back into billow and drift on the music's return. Alive. Area: `presets/InkBloom.tsx`. Accept: a quiet held bar visibly suspends the plumes, then they resume; kick bloom, snare shear, hat motes, gather pull, and tenderness milk (#87) stay intact. — PR #102
- [x] NS-20260725-10 (S) **Tide Veil kit split** — The caustic sheet rolls with swell and flashes on impact, but the drums pass through undifferentiated; wire `kick` as a deep caustic surge, `snare` as a lateral fold crack, and `hat` as pinpoint sparkles. Alive. Area: `presets/TideVeil.tsx`. Accept: kick/snare/hat each produce a distinct sheet accent; swell roll, gather fold, impact flash (#30), and holdBreath/tenderness (#71) stay intact. — PR #103

...
- [ ] NS-20260804-05 (S) **Halo Rain tension + dropEvent** — The rings drift, flare, tick, suspend, and near — but builds pass without a storm and drops without a payoff, its twice-dropped gap pile: on `tension`, tighten the ring spacing and darken the field as the build climbs, drift accelerating inward; on `dropEvent`, detonate one full-field ring wave — every ring flaring outward at once, then calming back to drift. Alive. Area: `presets/HaloRain.tsx`. Accept: a build visibly tightens and darkens the rings then springs loose, and the drop fires one full-field outward ring wave clearly bigger than the per-kick center pulse; drift + gather inhale + impact flare + hat ticks (#44), echo upward reverse rain (#70), kick center pulse + snare lateral shear (#100), holdBreath suspend + tenderness candle hush (#119), and leanIn approach zoom (#157) stay distinct.

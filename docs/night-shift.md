...
- [ ] NS-20260804-06 (S) **Ember Drift tension + dropEvent** — The ash rises, lifts, freezes, warms, and remembers — but builds pass flat and drops land without a blaze: on `tension`, press the ashfield low and dim it toward smoldering coals as the build climbs, sparks quickening underneath; on `dropEvent`, fire one full-field updraft flare — every ember surging upward and brightening at once, then settling back into drift. Alive. Area: `presets/EmberDrift.tsx`. Accept: a build visibly presses the ash low and dim then springs it loose, and the drop lifts the whole field once, clearly bigger than any per-kick lift; swell lift + gather inhale + impact flare + hat ticks (#54), kick lift + snare shear (#77), holdBreath freeze (#99), tenderness rosy drift (#112), echo spark train (#140), and leanIn densify + barPhase coal flicker (#167) stay distinct.

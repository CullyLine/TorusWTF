...
- [~] building 2026-07-20T11:01:31Z NS-20260720-01 (S) **SceneRig cinematic look SmoothDamp** — Cinematic shot cuts currently slam `lookAt` to the new target; ease look-target XYZ through a short SmoothDamp so framing glides into each cut while position springs already handle the body. Fluid camera across Torus/Star/Particle/Chrome defaults. Area: `SceneRig.tsx`. Accept: in cinematic mode, each cut’s look eases in (~0.2–0.35s) with no pop; once settled, look stays still between cuts.
- [ ] NS-20260720-02 (S) **Anima phrase-echo soul reply** — Kit accents already punch the core/rings; on `echo` fire a one-shot soul reply (brief core brighten + aurora counter-sweep) in phrase gaps so the creature answers when the music opens space. Alive. Area: `presets/Anima.tsx`. Accept: after a sung/lead phrase ends, one visible reply pulse/sweep; while the track is speaking, no echo reply; kick/snare/hat accents from #33 still distinct.
- [ ] NS-20260720-03 (M) **New preset: Ember Drift** — Fullscreen rising warm embers that lift on swell, inhale toward center on `gather`, flare on impact, and tick sparkle on `hat` — a fluid ashfield between Particle Storm chaos and Star Field dust. Area: new `presets/EmberDrift.tsx` + `registry.ts` (+ `controlSchema` only if one preset-owned slider). Accept: preset appears in the picker; owns the frame at defaults; mid/low tiers drop particle count without breaking.
- [ ] NS-20260720-04 (S) **Flow Field tenderness + convergence lock** — Kit currents already answer drums; on `tenderness` calm turbulence/trail jitter, and when `convergence` rises visibly tighten bandSpread so choruses read as one locked river. Alive cohesion. Area: `presets/FlowField.tsx` (uniforms / `flowParamsFromMetrics` only as needed). Accept: gentle vocal passage softens the storm; a chorus locks streams tighter than a breakdown; kick/snare/hat currents from #35 still readable.

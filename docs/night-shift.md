...
*(2026-08-01 — seventh consecutive full-speed day: eight of ten from 07-31 shipped and merged same day (#144–#151), including the roster's first underground preset. Today: the roster finally touches dry land — Dune Sea, a moonlit desert for a lineup that owns water, sky, space, underground, and forest but nothing arid; the day-old Glowworm Grotto learns anticipation and memory like Thunderhead, Murmuration, and Jellyfish Bloom did the day after they hatched; the bubble overlay gains the last engine-wide one-shot — phrase echo; `convergence` reaches Thunderhead (a storm organizing into a single cell is the sky's own lock-in); the two unbuilt carryovers (Alien Planet echo, Halo Rain leanIn) re-rank on proven patterns; and dual-metric passes close Tide Veil's leanIn, Ink Bloom's leanIn + dropEvent, Cosmic Mandala's tension + barPhase, and Outrun Grid's tenderness + vocalActivity — retiring four presets' final verified gaps. Dropped for now (still valid future gaps): Ember Drift / Paper Lanterns / Opal Slick / Night Bloom leanIn, Mist Spiral / Silk Wake echo, Murmuration / Jellyfish Bloom barPhase, Torus Field tension, Tide Veil vocalActivity, BackgroundLayer echo (deliberately skipped — three engine layers already answer the gap).)*

- [x] NS-20260801-01 (M) **New preset: Dune Sea** — A moonlit desert at night: rolling dune ridgelines of fine luminous sand under a deep indigo sky, wind-ripples crawling slowly along the slip faces. `kick` bursts a plume of sand off a dune crest — a different crest each strike so no two kicks look alike, grains arcing and settling with real gravity; `snare` drives a lateral wind gust that shears ripple lines across the field; `hat` winks tiny mica glints in the sand; `gather` stills the wind and swells the dunes on a pre-beat inhale; `tension` raises the wind and thickens a low sand-haze as the build climbs; `dropEvent` sweeps one full sandstorm veil across the scene; `tenderness` warms the moonlight toward honey; `holdBreath` drops the air dead-calm — drifting grains hang mid-air. The roster owns water, sky, space, underground, and forest — nothing dry, nothing sculpted by wind. One self-contained R3F component + registry entry (controls via `controlSchema.ts` if any). Area: `presets/DuneSea.tsx`, `registry.ts`. Accept: preset appears in the panel; on a drum-heavy track kicks visibly read as sand plumes off varying crests and a held quiet bar hangs the drifting grains mid-air; 60 fps on high tier, mid/low reduce grain/ridge counts gracefully. — PR #152
- [x] NS-20260801-02 (S) **Glowworm Grotto leanIn + phrase echo** — The day-old grotto cascades on kicks, sways on snares, and dims to embers — but never anticipates or remembers: on `leanIn`, the ceiling drifts subtly nearer and the thread tips brighten faintly, expectant, easing back on release (approach, distinct from tension's darkening and lengthening); on `echo`, fire a one-shot ghost cascade — a dim silver-blue pulse retraces the gap's rhythm down a single thread cluster, cooler and fainter than kick cascades. The newest preset learns anticipation and memory. Alive. Area: `presets/GlowwormGrotto.tsx`. Accept: a build-up visibly brings the ceiling nearer with brightening tips then releases, and a phrase gap fires one faint cool cascade clearly distinct from kick pulses and the dropEvent blaze; kick cascade / snare lag-sway / hat winks / gather inhale / tension deepen / tenderness amber / holdBreath embers (#144) stay distinct. — PR #153
- [x] NS-20260801-03 (S) **Bubble emitter phrase echo** — Engine-wide follow-up to #137/#148: the overlay hears the kit, the inhale, the hush, and the mood — but phrase gaps pass unanswered. On `echo`, release a one-shot train of small bright glint-bubbles from the base, spaced to the gap's rhythm, rising and fading as they climb — the column remembers the phrase. One-shot, subtle, no pile-up (presets' own echoes stay distinct because only one preset is active). Alive. Area: `emitters/bubbleSimulation.ts`, `emitters/BubbleEmitter.tsx`. Accept: with the emitter enabled over any preset, a phrase gap visibly releases one rhythmic train of brighter small bubbles that rises and fades; kick surge / snare flick / hat micro-glints / gather pull / holdBreath suspension / tenderness milky rise (#137/#148), breath/flow drift, and manual bursts unchanged. — PR #154
- [x] NS-20260801-04 (S) **Thunderhead convergence** — Real storms organize: scattered pocket flashes consolidating into one rhythmically pulsing cell. On `convergence`, settle the cloud's interior flicker into a single bar-locked heartbeat of light — the whole storm glowing and dimming with the band's shared groove — loosening back into scattered pocket strikes as the lock fades, no snap. The sky's own lock-in, sibling to Murmuration's sheet (#146) and Jellyfish Bloom's synchronized pulse (#147). Alive. Area: `presets/Thunderhead.tsx`. Accept: a locked-in groove visibly settles interior flickers into one shared rhythmic glow, then disperses; kick pocket lightning / snare rain shear / hat static / tension towering / dropEvent sky split (#134) and leanIn loom / echo sheet-lightning train (#145) stay distinct. — PR #155
- [x] NS-20260801-05 (S) **Alien Planet phrase echo** — The canopy rolls light rings, shears mist, and burns off fog — but never remembers a phrase: on `echo`, drift a one-shot train of cool bioluminescent spore glints across the crowns replaying the gap's rhythm — the alien cousin of Rainforest's firefly echo (#85) — fading as it goes. Alive. Area: `presets/AlienPlanet.tsx`. Accept: a phrase gap fires one visible cool glint train across the canopy, then it rests; kick light ring / snare mist shear (#81), gather mist stillness, tenderness warm bio hush (#113), and holdBreath windless mist stay distinct. — PR #156
- [x] NS-20260801-06 (S) **Halo Rain leanIn** — The rings fall, pulse, and suspend — but don't gather for the drop: on `leanIn`, tighten ring spacing and drift the field subtly nearer, expectant, loosening back on release (distinct from gather's upward reverse-inhale — this is approach). Alive. Area: `presets/HaloRain.tsx`. Accept: a build-up visibly tightens and nears the rings, then they loosen, no snap; gather reverse-inhale (#44), kick center pulse / snare shear (#100), echo upward reverse rain (#70), and holdBreath suspend + tenderness candle hush (#119) stay distinct. — PR #157
- [x] NS-20260801-07 (S) **Tide Veil leanIn** — The caustic sheet rolls, folds, flashes, and glints echo trains — but meets build-ups flat: on `leanIn`, raise the sheet subtly toward the camera and tighten the caustic folds, water gathering itself for the drop, easing back on release. Alive/beautiful. Area: `presets/TideVeil.tsx`. Accept: a build-up visibly nears the sheet with tightening folds then releases, no snap; swell roll / gather fold / impact flash / afterglow warmth (#30), kit split (#103), echo glint train (#131), and holdBreath hush + tenderness soften (#71) stay distinct. — PR #158
- [x] NS-20260801-08 (S) **Ink Bloom leanIn + dropEvent** — The plumes curl, shear, suspend, and ghost-replay — two verified gaps remain, and this closes both: on `leanIn`, coil the plumes tighter and drift them subtly nearer the glass, poised, uncoiling back on release; on `dropEvent`, erupt one full-water ink burst — every plume billowing at once, then settling. Anticipation and payoff for the tank. Alive. Area: `presets/InkBloom.tsx`. Accept: a build-up visibly coils and nears the plumes, and the drop erupts the whole tank once then settles; kick plumes / snare shear / hat motes / gather center / tenderness milk (#87), holdBreath suspension (#102), and echo ghost plumes (#132) stay distinct. — PR #159
- [x] NS-20260801-09 (S) **Cosmic Mandala tension + barPhase** — The mandala inhales, splits rings, ticks halos, and pauses — but freewheels through builds and drifts off the music's clock; this closes its last two verified gaps: on `tension`, darken the field and draw the ring spacing tighter and sharper as the build climbs, springing loose on the drop; with `barPhase`, phase-lock the slow halo rotation to the bar so the mandala turns with the music's own clock (continuous, no stepping). Fluid/alive. Area: `presets/CosmicMandala.tsx`. Accept: a build visibly darkens and tightens the rings then springs loose on the drop, and the rotation visibly breathes in time with the bar; gather inhale / kick-snare ring split / hat halo ticks / echo shimmer (#20), vocal rim + tender soften (#56), and holdBreath pause + leanIn approach (#118) stay distinct. — PR #160
- [~] building 2026-08-01T22:01:46Z NS-20260801-10 (S) **Outrun Grid tenderness + vocalActivity** — The grid punches, stretches, crawls, and leans — but has no gentle register, its last two verified gaps: on `tenderness`, ease into a dusk hush — the sun melts toward rose, grid glow softens, road speed relaxes; with `vocalActivity`, swell a warm rim of light around the sun core that breathes with the voice. Beautiful. Area: `presets/OutrunGrid.tsx`. Accept: a gentle vocal passage visibly softens the grid to a rose dusk while the sun's rim swells with the voice; kit dashes (#41), tension sun stretch + gather horizon dip (#21), echo ghost dash (#59), holdBreath road crawl (#110), and leanIn horizon approach (#142) stay distinct.

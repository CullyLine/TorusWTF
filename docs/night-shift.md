...
- [~] building 2026-07-19T16:01:06Z NS-20260719-05 (S) **Preset crossfade** — Switching presets crossfades the outgoing scene into the new one over ~0.35s instead of hard-cutting (respect `prefers-reduced-motion` → instant swap). Fluid and beautiful. Area: `VisualizerCanvas.tsx`. Accept: switching Torus Field → Anima crossfades cleanly; reduced-motion snaps; embedded clip-player behavior unchanged when preset is static.

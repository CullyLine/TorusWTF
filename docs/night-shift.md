...
- [~] building 2026-07-18T13:02:52Z NS-20260718-03 (S) **Infinite Tunnel kit accents** — Tunnel already gathers and echoes; add `kick` wall punch, `snare` lateral warp, and `hat` rail sparkle so the ride answers the drum kit. Alive depth cue. Area: `presets/InfiniteTunnel.tsx`. Accept: kick vs snare push different axes; hats glitter the side rails distinctly from bass wall explode.

...
- [~] building 2026-07-16T17:36:33Z NS-20260716-01 (M) **Camera spring inertia** — SceneRig camera modes no longer teleport to each frame’s target; position (and look-at for cinematic) ease through a critically-damped spring so orbit/drift/dive/flow glide and bass-shake rides on top without pops. Reads as fluid across every preset. Area: `SceneRig.tsx`. Accept: switch camera modes mid-track — motion eases, never snaps; heavy kick rumble still lands but settles smoothly.
- [ ] NS-20260716-02 (M) **Liquid Chrome call-and-response** — Chrome surface inhales on `gather`, releases on impact, rim sparkles on `hat`/`shimmer`, and quiet gaps replay faint `echo` ripples across the metal. Turns the gloss centerpiece from reactive lump into a listening body. Area: `presets/LiquidChrome.tsx`. Accept: on a four-on-the-floor track, watch a pre-beat squeeze then hit-release; hats glitter the rim; after a phrase gap, a delayed ripple passes.
- [ ] NS-20260716-03 (S) **Bloom musical breath** — Shared bloom intensity breathes with `swell`/`afterglow`, anticipates with `gather`, and still punches on impact/impulses — choruses glow, verses exhale, hits flash without strobing. Area: `SceneRig.tsx` (bloom frame loop). Accept: play a track with quiet verse → loud chorus; bloom lifts through the chorus and lingers via afterglow, not just on each kick.
- [ ] NS-20260716-04 (S) **Living palette mood warmth** — Palette life biases hue/saturation from `moodValence` + `tenderness` (warm vocal passages drift amber; cool instrumental valleys drift cyan) while keeping drop hue-kicks. Color itself feels emotional, not only loudness-driven. Area: `livingPalette.tsx`. Accept: vocal ballad vs cold techno — same base palette, clearly different warmth drift without jumpy hue.
- [ ] NS-20260716-05 (M) **Cosmic Mandala inhale & kit accents** — Whole mandala contracts on `gather`; outer rings crack on `snare`, inner pulse on `kick`, halo ticks on `hat`; `echo` briefly reverses shimmer drift in gaps. Sacred geometry that anticipates and answers the drum kit. Area: `presets/CosmicMandala.tsx`. Accept: pre-beat squeeze visible; kick vs snare hit different rings; hat twinkles the halo.
- [ ] NS-20260716-06 (M) **Outrun Grid build-and-drop cinema** — `tension` swells and stretches the sun, `gather` dips the horizon line, drop washes the grid in afterglow heat then eases back — synthwave that dramatizes builds instead of only scrolling faster. Area: `presets/OutrunGrid.tsx`. Accept: on a track with a clear build/drop, sun and grid visibly charge before the drop and bloom-wash on impact.

...
- [x] NS-20260716-06 (M) **Outrun Grid build-and-drop cinema** — `tension` swells and stretches the sun, `gather` dips the horizon line, drop washes the grid in afterglow heat then eases back — synthwave that dramatizes builds instead of only scrolling faster. Area: `presets/OutrunGrid.tsx`. Accept: on a track with a clear build/drop, sun and grid visibly charge before the drop and bloom-wash on impact. — PR #21
- [x] NS-20260716-07 (M) **Volumetric Waveform phrase echo** — Ribbon pinches toward the axis on `gather`, blooms on impact, and in post-phrase silence spawns a faded delayed crest from `echo` so the waveform answers the music. Minimal form, deeper life. Area: `presets/VolumetricWaveform.tsx`. Accept: pre-beat pinch; after a vocal phrase gap, a ghost crest travels the ribbon once. — PR #22

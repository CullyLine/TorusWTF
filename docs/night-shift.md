...
*(2026-07-23 — the Mad Scientist pack just landed on `main`: Rainforest Reverie, Alien Planet, Tidal Sanctuary, Galaxy Garden rewrite of `star_field`, Lava Choir rewrite of `liquid_blob`, and seven screen shaders. Top priority is folding these hero presets into the night-shift musical vocabulary — they all read kick/hat/gather/holdBreath but none answer `snare` or `echo`. Yesterday's Aura idea is dropped: `AuraLayer.tsx` on `main` already ships tenderness expansion + warmth.)*

- [x] NS-20260723-01 (S) **Rainforest Reverie snare canopy gust** — The flagship port answers kick (low thump), hats (canopy spec glitter), gather, holdBreath, and release, but snares pass silently; wire `snare` as a brief lateral wind-gust pulse through the canopy so the backbeat visibly brushes the trees. Alive. Area: `presets/RainforestReverie.tsx`. Accept: on a backbeat-heavy track, snares produce a distinct canopy brush vs the kick thump and hat glitter; gather / holdBreath / release sunburst / afterglow behavior unchanged. — PR #78
- [x] NS-20260723-02 (S) **Tidal Sanctuary snare crest crack** — The ocean swells on bass, rolls traveling kick waves (`uKickTravel`), sparkles on hats, but has no backbeat; wire `snare` as a short lateral whitecap crack — a foam/spray flash shearing along the crest line. Alive/beautiful. Area: `presets/TidalSanctuary.tsx`. Accept: kick keeps its traveling wave, snare produces a visibly different lateral foam crack, hat sparkle stays; holdBreath glassy calm unchanged. — PR #79
- [x] NS-20260723-03 (S) **Galaxy Garden phrase-echo swirl** — The new galaxy answers kick lensing, drop shockwaves, gather, holdBreath, and tenderness, but gaps stay empty; wire `echo` as a one-shot ghost counter-swirl of the spiral arms plus a brief star-glint replay so the galaxy answers the music in the gaps. Alive. Area: `presets/StarField.tsx`. Accept: after a phrase opens a gap, a one-shot reverse arm swirl + glint replay fires once; drop shockwave, kick lens, gather, holdBreath, and tenderness responses stay distinct. — PR #80
- [x] NS-20260723-04 (S) **Alien Planet snare canopy ripple** — Kick already rolls traveling light rings across the valley; wire `snare` as a lateral mist/canopy shear ripple so the drum trio is complete (hat spec glitter stays). Alive. Area: `presets/AlienPlanet.tsx`. Accept: kick rings and snare shear read as clearly different accents; gather / holdBreath / release sunburst / afterglow stay intact. — PR #81
- [x] NS-20260723-05 (S) **Background holdBreath sky hush** — Engine-wide: during `holdBreath` / deep `silence`, ease nebula/aurora drift speed and dim sky glitter so every backdrop holds still and listens, thawing on the music's return. Alive across all presets with a backdrop. Area: `BackgroundLayer.tsx`. Accept: a quiet held bar visibly stills the sky then it resumes; gather inhale (#24), afterglow warmth (#45), and leanIn pull + vocal warmth (#65) stay distinct. — PR #82
- [~] building 2026-07-23T22:01:09Z NS-20260723-06 (S) **Night Bloom kit accents** — The petals open on swell, inhale on gather, flare on impact, and mote-glitter on hats, but kick and snare pass silently; wire kick as a radial petal pulse and snare as a lateral petal shear so drums speak through the flower. Alive. Area: `presets/NightBloom.tsx`. Accept: kick/snare/hat each produce a distinct petal accent; swell open + gather inhale + impact flare from #76 stay intact.
- [ ] NS-20260723-07 (S) **Mandelbulb holdBreath stillness** — During `holdBreath` / deep `silence`, nearly freeze the fractal's morph/orbit motion so it listens, thawing on the music's return. Alive. Area: `presets/MandelbrotZoom.tsx`. Accept: a quiet held bar almost freezes the bulb then it resumes; gather dive, kick/snare accents, and phrase-echo orbit reverse from #31/#51 still fire when music returns.
- [ ] NS-20260723-08 (S) **Torus Field leanIn anticipation pull** — On `leanIn`, ease tube radius / presence slightly inward (anticipation) without overriding gather inhale, kit accents, or afterglow warmth. Alive. Area: `presets/TorusField.tsx`. Accept: pre-drop leanIn visibly pulls the field in; kick/snare/hat + echo reverse + afterglow from #29/#58 stay distinct.
- [ ] NS-20260723-09 (S) **Living palette leanIn cool anticipation** — Engine-wide: on `leanIn`, tilt the palette slightly cool and tighten saturation (the held-breath-before-the-drop look), releasing into the existing drop/impact warm bloom. Beautiful across every preset. Area: `livingPalette.tsx`. Accept: pre-drop passages read visibly cooler, then the drop warms as before; gather cool (#38), holdBreath hush (#57), and mood warmth (#19) stay distinct.
- [ ] NS-20260723-10 (S) **Silk Wake holdBreath hush + tenderness soften** — During `holdBreath`, slow braid travel and ease ribbon contrast; on `tenderness` soften ribbon sharpness/jitter so the silk listens on gentle vocals. Alive. Area: `presets/SilkWake.tsx`. Accept: quiet bar nearly stills braids then resumes; tender passage softens vs aggressive drop; kit accents + gather fold + afterglow from #36/#68 stay intact.

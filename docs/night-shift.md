...
- [~] building 2026-07-26T11:02:00Z NS-20260726-01 (S) **SceneRig tension coil** — Engine-wide: nothing in the rig answers `tension`, so build-ups pass without anticipation. As tension rises, the camera slowly coils — a couple degrees of FOV tighten plus a subtle upward creep, SmoothDamped over seconds — then springs loose on `dropEvent`/`release`. Every preset gets a held breath before every drop. Fluid/alive everywhere at once. Area: `SceneRig.tsx`. Accept: through a build-up the frame visibly tightens over several seconds and releases open on the drop with no snap; FOV hit punch (#25), leanIn/release Z (#62), cinematic pose cuts (#73), and echo sway (#97) stay distinct.
- [ ] NS-20260726-02 (M) **New preset: Paper Lanterns** — A night flotilla of glowing paper lanterns rising over dark water, their light mirrored in the swell below. Lanterns bob and rise with buoyant inertia (never linear); `kick` flares the flames and surges the lift, `snare` is a lateral gust that sways the flotilla, `hat` sparks tiny ember ticks, `gather` draws the lanterns toward center, `tenderness` softens everything to honey-warm. Warm points of light over water — the most literal "alive/beautiful" on the list. One self-contained R3F component + registry entry (controls via `controlSchema.ts` if any). Area: `presets/PaperLanterns.tsx`, `registry.ts`. Accept: preset appears in the panel; lanterns rise and bob with visible inertia and mirrored water glow at 60 fps on high tier; mid/low tiers degrade gracefully (fewer lanterns, simpler reflections).

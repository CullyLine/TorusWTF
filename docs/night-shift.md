...
- [ ] NS-20260720-01 (S) **SceneRig cinematic look SmoothDamp** — Cinematic shot cuts currently slam `lookAt` to the new target; ease look-target XYZ through a short SmoothDamp so framing glides into each cut while position springs already handle the body. Fluid camera across Torus/Star/Particle/Chrome defaults. Area: `SceneRig.tsx`. Accept: in cinematic mode, each cut’s look eases in (~0.2–0.35s) with no pop; once settled, look stays still between cuts.

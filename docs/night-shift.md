...
- [ ] NS-20260718-01 (S) **Aura flock to the beat** — Aura wisps drift inward on `gather`, burst outward on impact/`release`, and brighten with `shimmer`/`hat` — the presence layer flocks with the music instead of only wandering on Perlin paths. Alive across every preset. Area: `AuraLayer.tsx`. Accept: with Aura up, wisps visibly inhale before kicks and glitter on hats on any preset.

...
- [x] NS-20260721-05 (S) **Particle Storm tenderness calm + vocal tint** — Kit whip + echo reverse already ship; on `tenderness` calm swirl/jitter and bias particle color toward a softer vocal-warm mix with `vocalActivity`. Alive cohesion. Area: `presets/ParticleStorm.tsx`. Accept: gentle vocal passage softens the storm and warms tint vs harsh instrumental; kick/snare/hat whip + echo reverse from #28 still readable. — PR #66
- [x] NS-20260721-06 (S) **Volumetric Waveform holdBreath freeze + tenderness soften** — During `holdBreath`, nearly freeze crest travel / pinch motion; on `tenderness` soften ridge sharpness and height jitter so the waveform listens. Alive. Area: `presets/VolumetricWaveform.tsx`. Accept: quiet held bar almost stills the form then resumes; tender vocal passage softens ridges; gather pinch + kit accents + echo ghost from #22/#40 still work. — PR #67
- [~] building 2026-07-21T19:02:27Z NS-20260721-07 (S) **Silk Wake kit accents** — Braided ribbons currently answer gather/impact/afterglow only; wire kick thrust along braid, snare lateral shear flash, and hat mote ticks so drums speak through the silk. Alive. Area: `presets/SilkWake.tsx`. Accept: kick/snare/hat each produce a distinct ribbon accent; gather fold + impact flare + afterglow trails from #36 stay intact.

...
- [~] building 2026-07-25T11:05:00Z NS-20260725-01 (S) **Living palette phrase-echo hue glint** — Engine-wide: on `echo` (the phrase-gap replay), run a one-shot subtle hue shimmer / saturation glint that pulses with the replayed rhythm then settles, so every preset's color answers the gaps between phrases. Alive/beautiful everywhere at once. Area: `livingPalette.tsx`. Accept: when a phrase gap opens, colors glint once in rhythm and settle; gather cool (#38), leanIn cool (#84), holdBreath hush (#57), and mood warmth (#19) stay distinct.
- [ ] NS-20260725-02 (S) **Rainforest Reverie tenderness misty hush** — The flagship answers the full kit, gather, holdBreath, release, and echo, but it never goes gentle; on `tenderness`, ease canopy sway and lay a warm soft-mist veil over the light so tender vocal passages gentle the forest while it keeps breathing. Alive/beautiful. Area: `presets/RainforestReverie.tsx`. Accept: a tender vocal passage visibly calms and warms the forest and reads different from holdBreath stillness; kick thump, snare gust (#78), hat glitter, and echo fireflies (#85) stay distinct.
- [ ] NS-20260725-03 (M) **New preset: Opal Slick** — A dark rain puddle seen close-up: a thin-film interference sheen swirls slow spectral rainbows across the surface; kicks drop ripple rings that bend the sheen, snares shear the film laterally, hats spark micro-glints, gather pulls the swirl toward center, tenderness milkens the film toward pearl. The most literal "beautiful fluid" on the list. One self-contained R3F component + registry entry (controls via `controlSchema.ts` if any). Fluid/beautiful. Area: `presets/OpalSlick.tsx`, `registry.ts`. Accept: preset appears in the panel; kick ripples visibly bend and refract the rainbow sheen at 60 fps on high tier; mid/low tiers degrade gracefully.
- [ ] NS-20260725-04 (S) **SceneRig phrase-echo parallax sway** — Engine-wide: on `echo`, run a one-shot slow lateral camera drift-and-return (SmoothDamp out and back) so the camera itself answers phrase gaps with a gentle look-around, then settles. Fluid/alive on every preset. Area: `SceneRig.tsx`. Accept: in a phrase gap the camera sways once and settles with no snap; kit accents (#23), leanIn Z (#62), cinematic pose cuts (#73), and shake stay distinct.
- [ ] NS-20260725-05 (S) **Tidal Sanctuary phrase-echo moonlit glint replay** — The sea answers the kit, gather, holdBreath, and tenderness, but the gaps after phrases stay dark; wire `echo` as a one-shot train of moonlit glints traveling across the swell, pulsing with the replayed rhythm. Alive. Area: `presets/TidalSanctuary.tsx`. Accept: after a phrase opens a gap, glints travel the surface in rhythm once then fade; kick travel wave, snare crest crack (#79), hat sparkle, and tenderness pearl (#86) stay distinct.
- [ ] NS-20260725-06 (S) **Ember Drift holdBreath suspension** — During `holdBreath` / deep `silence`, the rising embers freeze mid-air and their glow dims toward coals, thawing upward on the music's return. The most poetic stillness on the roster. Alive. Area: `presets/EmberDrift.tsx`. Accept: a quiet held bar visibly suspends the ash field, then it resumes rising; kick lift, snare shear (#77), hat ticks, and gather/impact/afterglow (#54) stay intact.
- [ ] NS-20260725-07 (S) **Halo Rain kick/snare ring split** — The rings drift, flare on impact, tick on hats, and reverse on echo, but kick and snare share one undifferentiated flare; split them — `kick` as a center-born ring pulse, `snare` as a lateral ring shear flash. Alive. Area: `presets/HaloRain.tsx`. Accept: kick and snare each produce a visibly distinct ring accent; hat ticks (#44) and echo reverse rain (#70) stay intact.
- [ ] NS-20260725-08 (S) **Background tenderness sky soften** — Engine-wide: on `tenderness`, ease nebula/aurora drift and warm-dim the glow so the sky goes gentle under tender vocals, re-brightening on release. Alive across all presets. Area: `BackgroundLayer.tsx`. Accept: a tender passage visibly softens and warms the sky vs an aggressive section; gather inhale (#24), holdBreath hush (#82), leanIn pull (#65), and afterglow warmth (#45) stay distinct.

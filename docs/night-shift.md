...
- [x] NS-20260721-01 (S) **SceneRig leanIn/release Z SmoothDamp** — `leanIn` / `release` currently add raw Z offsets onto `desiredZ` every frame; SmoothDamp those choreography offsets (~0.18–0.28s) so anticipation dollies and drop exhales glide instead of stair-stepping through the pose spring. Fluid camera across every preset. Area: `SceneRig.tsx`. Accept: on a build→drop, lean-in and release Z ease without jitter; quiet passages settle still; existing pose springs + shake SmoothDamp unchanged. — PR #62
- [x] NS-20260721-02 (S) **Bloom holdBreath soft dim** — During `holdBreath` / deep `silence`, ease bloom breath target down a notch so the shared glow listens with the creature (swell/afterglow/gather/hit paths stay). Beautiful + alive engine-wide. Area: `SceneRig.tsx` (bloom breath only). Accept: held quiet bar dims bloom slightly then restores on music return; kick/hit bloom flashes and gather lift from #18 still fire. — PR #63
- [x] NS-20260721-03 (M) **New preset: Mist Spiral** — Fullscreen rising mist coils that spiral around a vertical axis, inhale toward center on `gather`, flare on impact, and mote-glitter on `hat` — a soft atmospheric field between Ember Drift ash and Tide Veil caustics. Area: new `presets/MistSpiral.tsx` + `registry.ts` (+ `controlSchema` only if one preset-owned slider). Accept: preset appears in the picker; owns the frame at defaults; mid/low tiers drop coil/octave count without breaking. — PR #64
- [x] NS-20260721-04 (S) **Background leanIn sky pull + vocal warmth** — On `leanIn`, ease nebula/aurora/glow slightly toward camera/center (anticipation); tint sky a soft voice-warm mix with `vocalActivity` without overriding afterglow amber linger. Alive sky. Area: `BackgroundLayer.tsx`. Accept: pre-drop leanIn visibly pulls the sky in; vocal-led verse warms vs instrumental; gather inhale (#24) and afterglow warmth (#45) still distinct. — PR #65
- [~] building 2026-07-21T16:01:19Z — NS-20260721-05 (S) **Particle Storm tenderness calm + vocal tint** — Kit whip + echo reverse already ship; on `tenderness` calm swirl/jitter and bias particle color toward a softer vocal-warm mix with `vocalActivity`. Alive cohesion. Area: `presets/ParticleStorm.tsx`. Accept: gentle vocal passage softens the storm and warms tint vs harsh instrumental; kick/snare/hat whip + echo reverse from #28 still readable.
- [ ] NS-20260721-06 (S) **Volumetric Waveform holdBreath freeze + tenderness soften** — During `holdBreath`, nearly freeze crest travel / pinch motion; on `tenderness` soften ridge sharpness and height jitter so the waveform listens. Alive. Area: `presets/VolumetricWaveform.tsx`. Accept: quiet held bar almost stills the form then resumes; tender vocal passage softens ridges; gather pinch + kit accents + echo ghost from #22/#40 still work.
- [ ] NS-20260721-07 (S) **Silk Wake kit accents** — Braided ribbons currently answer gather/impact/afterglow only; wire kick thrust along braid, snare lateral shear flash, and hat mote ticks so drums speak through the silk. Alive. Area: `presets/SilkWake.tsx`. Accept: kick/snare/hat each produce a distinct ribbon accent; gather fold + impact flare + afterglow trails from #36 stay intact.
- [ ] NS-20260721-08 (S) **Liquid Blob holdBreath surface stillness** — During `holdBreath` / deep `silence`, ease deformation speed and satellite chatter so the organism holds still and listens (gather inhale + kit axes unchanged). Alive. Area: `presets/LiquidBlob.tsx`. Accept: quiet bar nearly freezes surface motion then thaws; kick Y / snare X / gather / echo from #27/#47 still fire when music returns.
- [ ] NS-20260721-09 (S) **Halo Rain phrase-echo reverse rain** — On `echo`, fire a one-shot upward rain reverse / ring after-image in phrase gaps so the halos answer when the music opens space. Alive call-response. Area: `presets/HaloRain.tsx`. Accept: gap after a phrase shows one reverse-rain reply; while drums speak, no echo reply; gather inhale + impact flare + hat ticks from #44 stay distinct.
- [ ] NS-20260721-10 (S) **Tide Veil holdBreath hush + tenderness soften** — During `holdBreath`, slow caustic roll and ease ridge contrast; on `tenderness` soften caustic sharpness so the veil listens on gentle vocals. Alive. Area: `presets/TideVeil.tsx`. Accept: quiet bar nearly stills the sheet then resumes; tender passage softens ridges vs aggressive drop; gather fold + impact flash + afterglow from #30 unchanged.

...
- [~] NS-20260721-03 (M) **New preset: Mist Spiral** — Fullscreen rising mist coils that spiral around a vertical axis, inhale toward center on `gather`, flare on impact, and mote-glitter on `hat` — a soft atmospheric field between Ember Drift ash and Tide Veil caustics. Area: new `presets/MistSpiral.tsx` + `registry.ts` (+ `controlSchema` only if one preset-owned slider). Accept: preset appears in the picker; owns the frame at defaults; mid/low tiers drop coil/octave count without breaking. building 2026-07-21T13:02:17Z
- [ ] NS-20260721-04 (S) **Background leanIn sky pull + vocal warmth** — On `leanIn`, ease nebula/aurora/glow slightly toward camera/center (anticipation); tint sky a soft voice-warm mix with `vocalActivity` without overriding afterglow amber linger. Alive sky. Area: `BackgroundLayer.tsx`. Accept: pre-drop leanIn visibly pulls the sky in; vocal-led verse warms vs instrumental; gather inhale (#24) and afterglow warmth (#45) still distinct.

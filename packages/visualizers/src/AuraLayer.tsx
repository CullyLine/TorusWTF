  ...
  useFrame((_state, delta) => {
    ...
    if (points && mat) {
      ...
      for (let i = 0; i < wispCount; i++) {
        ...
        x += Math.sin(now * fx + i * 0.13) * delta * 0.08 * wanderScale;
        y += Math.cos(now * fy + i * 0.17) * delta * 0.06 * (1 + m.bass * 0.4) * wanderScale;
        z += Math.sin(now * fz + i * 0.21) * delta * 0.04 * wanderScale;
        ...
        const radial = (flockOut - flockIn) * phase * delta;
        x += dx * invR * radial;
        y += dy * invR * radial;
        z += dz * invR * radial;
        ...
        if (escapeR > 6) {
          const pull = (escapeR - 6) * delta * 0.5;
